...
public class FileStore implements TokenStore
{
	...
	private String[] headers = new String[] { Constants.ID, Constants.USER_NAME, Constants.CLIENT_ID, Constants.CLIENT_SECRET, Constants.REFRESH_TOKEN, Constants.ACCESS_TOKEN, Constants.GRANT_TOKEN, Constants.EXPIRY_TIME, Constants.REDIRECT_URL };
			...
			for (int index = 1; index < allContents.size(); index++)
			{
				...
				if (nextRecord.length > 1)
				{
					if (oauthToken.getUserSignature() != null)
					{
						...
						String userName = (nextRecord[1] != null && !nextRecord[1].isEmpty() && nextRecord[1].length() > 0) ? nextRecord[1] : null;
					...
					{
						String accessToken = (nextRecord[5] != null && !nextRecord[5].isEmpty() && nextRecord[5].length() > 0) ? nextRecord[5] : null;
		...
		try (CSVReader csvReader = new CSVReader(new FileReader(this.filePath)))
		{
			...
			for (int index = 1; index < allContents.size(); index++)
			{
				...
				if (nextRecord.length > 1)
				{
					...
					else if (oauthToken.getUserSignature() != null)
					{
						String name = oauthToken.getUserSignature().getName();
		...
		try (CSVReader csvReader = new CSVReader(new FileReader(this.filePath)))
		{
			...
			for (int index = 1; index < allContents.size(); index++)
			{
				...
				if (nextRecord.length > 1)
				{
					...
					String redirectURL = getData(nextRecord[8]);
					...
					oauthToken.setUserSignature(new UserSignature((nextRecord[1])));
