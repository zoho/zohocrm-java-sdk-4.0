...
public class Utility
{
	...
	{
		RelatedListsOperations relatedListsOperations = new RelatedListsOperations(moduleAPIName, null);
	...
	private static void setDataType(JSONObject fieldDetail, Field field, String moduleAPIName) throws SDKException
	{
		...
		com.zoho.crm.api.fields.ModuleMap module = new com.zoho.crm.api.fields.ModuleMap();
		...
		if (apiType.toLowerCase().equalsIgnoreCase(Constants.MULTI_SELECT_LOOKUP))
		{
			...
			if (field.getMultiselectlookup() != null)
			{
				...
				{
					com.zoho.crm.api.fields.ModuleMap linkingModule = multiselectlookup.getLinkingModule();
		...
		if (apiType.toLowerCase().equalsIgnoreCase(Constants.MULTI_USER_LOOKUP))
		{
			...
			if (field.getMultiuserlookup() != null)
			{
				...
				{
					com.zoho.crm.api.fields.ModuleMap linkingModule = multiuserlookup.getLinkingModule();
		...
		{
			module.setAPIName(((Subform) field.getSubform()).getModule());
		...
		{
			com.zoho.crm.api.fields.Module module1 = ((Lookup) field.getLookup()).getModule();
			...
			{
				module = new com.zoho.crm.api.fields.ModuleMap();
			}
		...
		{
			Utility.getFields(module.getAPIName(), null);
		}
