...
public class OAuthToken implements Token
{
	...
	private OAuthToken refreshAccessToken(OAuthToken oauthToken, TokenStore store, String url) throws SDKException
	{
	...
	private OAuthToken generateAccessToken(OAuthToken oauthToken, TokenStore store, String url) throws SDKException
	{
		HashMap<String, String> requestParams = new HashMap<String, String>();
	...
	private void parseResponse(OAuthToken oauthToken, String response) throws SDKException
	{
		try
		{
			JSONObject responseJSON = new JSONObject(response);
	...
	public static <T> void mergeObjects(T first, T second) throws SDKException
	{
		try
		{
	...
	public static class Builder
	{
		...
		public OAuthToken build() throws SDKException
		{
			...
			return new OAuthToken(this.clientID, this.clientSecret, this.grantToken, this.refreshToken, this.redirectURL, this.id, this.accessToken, this.userSignature);
		}
	}
}
