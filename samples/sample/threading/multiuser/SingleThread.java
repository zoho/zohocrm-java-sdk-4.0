...
public class SingleThread
{
	...
	public void run()
	{
		try
		{
			new Initializer.Builder().user(user).environment(environment).token(token).SDKConfig(sdkConfig).switchUser();
	...
	public static void main(String[] args) throws Exception
	{
		...
		UserSignature user1 = new UserSignature("abck@zoho.com");

		TokenStore tokenstore = new DBStore.Builder().build();

		Token token1 = new OAuthToken.Builder().clientID("1000.xxxxxx").clientSecret("xxxxx").refreshToken("1000.xxxxx.xxxxx").redirectURL("https://www.zoho.com").build();
		...
		Token token2 = new OAuthToken.Builder().clientID("1000.xxxx").clientSecret("xxxxx").refreshToken("1000.xxxxx.xxxx").build();
