...
public class SingleThread extends Thread
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		Token token1 = new OAuthToken.Builder().clientID("1000.xxxx").clientSecret("xxxx").refreshToken("1000.xxxx.xxxx").redirectURL("https://www.zoho.com").build();

		String resourcePath = "/usr/";

		RequestProxy userProxy = new RequestProxy.Builder().host("proxyHost").port(80).user("proxyUser").password("password").userDomain("userDomain").build();

		SDKConfig sdkConfig = new SDKConfig.Builder().autoRefreshFields(false).pickListValidation(true).build();

		new Initializer.Builder().user(user).environment(env).token(token1).store(tokenstore).SDKConfig(sdkConfig).resourcePath(resourcePath).logger(loggerInstance).requestProxy(userProxy).initialize();
