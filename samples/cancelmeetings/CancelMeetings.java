...
{
	public static void cancelMeetings(Long eventId) throws Exception
	{
		CancelMeetingsOperations cancelMeetingsOperations = new CancelMeetingsOperations(eventId);
		BodyWrapper request = new BodyWrapper();
		List<Notify> data = new ArrayList<>();
		Notify notify = new Notify();
		notify.setSendCancellingMail(Boolean.FALSE);
		data.add(notify);
	...
	public static void main(String[] args)
	{
		try
		{
			...
			Long eventId = 347706116709021l;
			cancelMeetings(eventId);
		}
		catch (Exception e)
		{
			e.printStackTrace();
